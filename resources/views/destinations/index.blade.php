                        ...
                        <ul class="nav nav-tabs align-items-end card-header-tabs w-100">
                            ...
                            <li class="nav-item">
                                ...
                                <a class="nav-link active" href="{!! url()->current() !!}"><i
                                        class="fa fa-list mr-2"></i>{{trans('lang.destination_table')}}</a>

                            </li>

                            <li class="nav-item">

                                <a class="nav-link" href="{!! route('destinations.create') !!}"><i
                                        class="fa fa-plus mr-2"></i>{{trans('lang.destination_create')}}</a>
...
function buildHTML(snapshots) {
    ...
    snapshots.docs.forEach(async (listval) => {
        ...
        html = html + '<td class="vendor-action-btn"><a href="' + route1 +
            '"><i class="fa fa-edit"></i></a><a id="' + val.id +
            '" name="vendor-delete" class="do_not_delete" href="javascript:void(0)"><i class="fa fa-trash"></i></a></td>';
...
async function next() {
    ...
    if (start != undefined || start != null) {
        ...
        if (jQuery("#selected_search").val() == 'title' && jQuery("#search").val().trim() != '') {
            ...
            listener = refData.orderBy('title').limit(pagesize).startAt(jQuery("#search").val()).endAt(jQuery(
                "#search").val() + '\uf8ff').startAfter(start).get();
...
function searchtext() {
    ...
    if (jQuery("#selected_search").val() == 'title' && jQuery("#search").val().trim() != '') {
        ...
        wherequery = refData.orderBy('title').limit(pagesize).startAt(jQuery("#search").val()).endAt(jQuery("#search")
            .val() + '\uf8ff').get();
...
$(document).on("click", "a[name='vendor-delete']", function(e) {
    jQuery("#data-table_processing").show();
    var id = this.id;
    database.collection('popular_destinations').doc(id).delete().then(function() {
    ...
    if ($('#example24 .is_open:checked').length) {
        if (confirm("{{trans('lang.selected_delete_alert')}}")) {
            ...
            $('#example24 .is_open:checked').each(function() {
                ...
                console.log(dataId);
                database.collection('popular_destinations').doc(dataId).delete().then(function() {
                        setTimeout(function(){
                            window.location.reload();
                        },5000);
                });
