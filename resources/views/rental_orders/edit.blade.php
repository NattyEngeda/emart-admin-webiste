        ...
        <div class="col-md-7 align-self-center">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a
                            href="{{url('/rental_orders')}}">{{trans('lang.rental_plural')}}
            ...
            <div class="order_detail-top">
                <div class="row">
                    ...
                    <div class="order_addre-edit col-md-4">
                        <h3>{{ trans('lang.pick_up_details')}}</h3>
                    ...
                    <div class="order_addre-edit col-md-4">
                        <h3>{{ trans('lang.drop_off_details')}}</h3>
...
$(document).ready(function () {
    ...
    $(".save_order_btn").click(async function () {
        ...
        if (old_order_status != orderStatus) {
            ...
            database.collection('rental_orders').doc(id).update({'status': orderStatus}).then(async function (result) {
                if (orderStatus != orderPreviousStatus && payment_shared == false) {
                    if (orderStatus == 'Order Completed') {

                        await database.collection('rental_orders').doc(id).update({'payment_shared': true}).then(async function (result) {
...
function buildParcelTotal(snapshotsProducts) {
    ...
    if (currencyAtRight) {
        ...
        html = html + '<tr><td class="label">{{trans("lang.sub_total_plural")}}</td><td>  ' + parseFloat(total_price).toFixed(decimal_degits) + "" + currentCurrency + '</td></tr>';

    } else {

        html = html + '<tr><td class="label">{{trans("lang.sub_total_plural")}}</td><td>  ' + currentCurrency + "" + parseFloat(total_price).toFixed(decimal_degits) + '</td></tr>';
