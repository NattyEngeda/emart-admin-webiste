...
function buildHTML(snapshots) {
    ...
    alldata.forEach((listval) => {
        ...
        if (val.orderId) {
            const ride = rideDetails(val.orderId);
            html = html + '<td data-url="' + trroute1 + '"  class="redirecttopage ride_client' + val.orderId + '"></td>';
            html = html + '<td data-url="' + trroute1 + '"  class="redirecttopage ride_driver' + val.orderId + '"></td>';
            html = html + '<td data-url="' + trroute1 + '"  class="redirecttopage ride_address' + val.orderId + '"></td>';
        }
...
async function rideDetails(ride) {
    var rideDetails = '';
    await database.collection('rides').where("id", "==", ride).get().then(async function (snapshotss) {
        var rideroute = '{{route("rides.edit",":id")}}';
        rideroute = rideroute.replace(':id', ride);

        if (snapshotss.docs[0]) {
            var ride_data = snapshotss.docs[0].data();
            client_name = ride_data.author.firstName;
            driver_name = ride_data.driver.firstName;
            address = ride_data.destinationLocationName;
            jQuery(".ride_client" + ride).html(client_name);
            jQuery(".ride_driver" + ride).html(driver_name);
            jQuery(".ride_address" + ride).html(address);
        } else {
            jQuery(".ride_client" + ride).html('');
            jQuery(".ride_driver" + ride).html('');
            jQuery(".ride_address" + ride).html('');
        }
    });
    return rideDetails;
}
