...
<script type="text/javascript">
    ...
    var refData = database.collection('brands');
    ...
    $(document).on("click","input[name='publish']",function(e){
        ...
        var id=this.id;
        if(ischeck){
            database.collection('brands').doc(id).update({'is_publish': true}).then(function (result) {

            });
        }else{
            database.collection('brands').doc(id).update({'is_publish': false}).then(function (result) {
